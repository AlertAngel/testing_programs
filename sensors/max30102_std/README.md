...
This shows how to use a MAX30102 HEart Rate and Pulse Oximeter sensor using `std` and Embedded Rust
...
use hayasen::{max30102_hayasen, mpu6050};
...
impl HeartRateDetector {
    ...
    fn dc_removal(&mut self, x: i32) -> i32 {
        let w = x + (31 * self.dc_filer_w) / 32;
        let result = w - self.dc_filer_w;
        self.dc_filer_w = w;
        result
    }
...
fn main() -> Result<()> {
    ...
    let mut sensor = match max30102_hayasen::create_default_with_address(i2c) {
        Ok(mut s) => {
            info!("Sensor initialized successfully");
            let _ = max30102_hayasen::setup_high_performance_mode(&mut s);
            s
        },
        Err(_) => {
    ...
    loop {
        if let Ok(count) = max30102_hayasen::read_fifo_batch(&mut sensor, &mut sample_buffer) {
            if count > 0 {
                for i in 0..count {
        ...
        if temp_counter >= 250 {
            temp_counter = 0;
            let _ = max30102_hayasen::start_temperature_measurement(&mut sensor);
            delay.delay_ms(30);

            if let Ok(Some(temp)) = max30102_hayasen::read_temperature(&mut sensor) {
                current_temp = temp;
            }
        }
